...
class TestIncomes(unittest.TestCase):
    def test_sum_NPP_income(self):
        ...
        df = pd.DataFrame(
            {"Kod": [75621, 75621], "Dochody PIT": [100, 1], "NPP": ["a", "a"]}
        )
    ...
    def test_set_and_apply_tax_rate(self):
        ...
        df = pd.DataFrame(
            {"NPP": ["aaV", "aaV"], "ROZDZIAŁ": [75621, 75622], "Dochody PIT": [100, 1]}
        )
        df.attrs["JST_type"] = "NPP"
        df = set_and_apply_tax_rate(df, "2020", None, None)
    ...
    def test_set_JST_type(self):
        ...
        df = set_JST_type(
            pd.DataFrame({"WK": ["20", "30"], "JST": ["20", "5"]}),
    ...
    def test_merge_territorial_code(self):
        ...
        df = merge_territorial_code(
            pd.DataFrame({"WK": ["20", "30"], "GK": ["1", "-"], "xd": ["20", "5"]})
        )
    ...
    def test_drop_na_columns(self):
        ...
        df = drop_na_columns(
            pd.DataFrame({"xd": [None, None, None, None, None, None, None]})
        )
